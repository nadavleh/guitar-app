...
- **Tonic banner.** A progression with no `I` of its own is flagged directly under the bar squares — but in **two** distinct flavours, because conflating them was wrong. One that **ends on the relative tonic** (`IV–V–iii–vi` finishes on `vi` = the relative minor's `i`) resolves perfectly well: it is a minor-key progression wearing major numerals, so it gets a **neutral** `◆ READS IN THE RELATIVE MINOR ◆` card that states the minor reading (`bVI – bVII – v – i`); the library/drill marker is `◆ relative minor`. Only one that never lands anywhere (`vi–V–IV–V` hangs on `V`) keeps the **error-tinted** `◆ NO TONIC ◆` warning and the `◆ no-tonic (hard)` marker. The relative Roman line names every bar, so it **is the answer** — it is printed in Practice and the library, and in Challenge only once every bar already carries a verdict (`challengeAllBarsAnswered`).
