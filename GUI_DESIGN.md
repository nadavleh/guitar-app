...
- **Tonic banner.** A progression with no `I` of its own is flagged directly under the bar squares — in **two** distinct flavours, keyed on whether it **contains** the relative tonic (v2.78.1; keying on whether it *ended* there was wrong). Any progression holding the relative tonic anywhere has a home — `IV–V–iii–vi` finishes on `vi` and `vi–V–IV–V` **opens** on it, and both `vi` **are** the relative minor's `i` — so it gets the **neutral** `◆ READS IN THE RELATIVE MINOR ◆` card (`… MAJOR` for a minor progression carrying `bIII`) stating the relative reading, with the library/drill marker `◆ relative minor` / `◆ relative major`. The body line says which of the two it is: *"it lands on the relative tonic"* for `IV–V–iii–vi` → `bVI – bVII – v – i`, or *"bar 1 IS the relative tonic — it just doesn't end there"* for `vi–V–IV–V` → `i – bVII – bVI – bVII`. Only a progression holding **neither** tonic (e.g. `ii–V–IV–V`) keeps the **error-tinted** `◆ NO TONIC ◆` warning and the `◆ no-tonic (hard)` marker. The relative Roman line names every bar, so it **is the answer** — it is printed in Practice and the library, and in Challenge only once every bar already carries a verdict (`challengeAllBarsAnswered`).
...
- **Translate row**: a loop-shift control that rotates the whole loop by ±n slots with **wrap-around** — `◀` / `▶` shift by one slot, or the user types an amount in a small field and presses **Go** to shift by that many slots.
