...
- **Data (v2.74)** — the **only** place recorded practice history can be erased, and each list has its own button:
  - `Challenge stats — N recorded runs` → `Delete single runs…` (expands a per-run list, each with a `✕`) and `Delete all` (behind a confirm).
  - `Drill list — N progressions tracked` → `Clear drill list` (behind a confirm).

  The two are **independent by design**: the score log records what you did, the Drill list is a curated set of what you still get wrong, so clearing one must never silently throw away the other — both button captions say so. This moved here **out of** the Challenge-stats popup, which is now **read-only** (its `Clear all` / per-kind `Clear` / per-row `✕` are gone, replaced by the line "To delete runs, open Settings → Data"): those buttons sat a thumb-width from a score you had just set, and hitting one wiped a history that took weeks to build.
...
- **Center dial**: a quarter-ring (90° sweep, apex at north) spanning ±50 cents, with major/minor/micro cent ticks and a pivoting needle. The big detected-note label sits in the middle and turns **green + "IN TUNE"** within ±10¢. The note label is **tappable** → plays the equal-tempered reference tone and locks the dial to "spot on" for the ring-sustain duration. In **portrait** the dial is constrained to a compact **1.1:1** box and centered; in landscape it fills the space. The ring's radius is `min(0.66 * w, 0.72 * h)` and every stroke, tick and the needle scale with it (`k = radius / 450`), so the gauge keeps its proportions at any size. Both numbers were retuned in v2.72.4 after measuring on a phone: at the old 1.5:1 box and `0.46 * w` radius, a 90° arc spans only `1.41 * r` — about 65% of the width — which read as a thin sliver floating in a mostly empty column. If the mic permission is missing, a `MicPermissionPanel` with a Retry button shows instead of the dial.
...
- In-flight: `Question k / 15`, running `Score: N bars`, `Restart`, `Quit`; a **top-pinned `← Prev` / `Next →` navigation row**; a transport row (Play/Stop, `Hear <cadence>`, `Re-roll`); a BPM slider; an optional low-emphasis `Key & Mode (hint)` reveal chip. (The `Restart` action restarts the run from question 1 without finishing it; it sits next to `Quit` in **every** sub-mode's challenge header, not just Progressions.)
- **Per-question history + Prev/Next.** The challenge keeps a per-question history. `← Prev` returns to an earlier question and **restores its saved answers** (so an accidental `Next` is recoverable); `Next →` advances. These are pinned at the **top** — previously only a bottom `Next` button existed.
- **"Hear the degrees"** reference palette — a FlowRow of `▶ <n>` buttons that audition each diatonic degree *in the hidden key*. The buttons are labelled with **plain numbers** (`1..7` / positional) rather than Roman numerals, so auditioning a degree doesn't reveal whether the key is major or minor. These plus the per-bar `▶ Play` are the **only** things that make sound; selecting an answer never plays a chord (so you compare candidates deliberately).
- **Per-bar degree keyboard (default answering tool):** each of the 4 bars is rendered as a **square** the user taps to open a degree **keyboard** — a row of scale-degree keys with a Major/Minor **shift** that relabels the same seven shared diatonic chords, so relative-major/minor answers (e.g. major I–IV–V vs minor III–VI–VII) are accepted as **equivalent**. An **extensions row** is shown only when the level uses 7ths/extensions; triads get a simplified keyboard. The bar **auto-scores** on selection — the square turns green/red and reveals `✔/✘ answer`. (This replaced the earlier inline answer chips.)
- **Answer keyboard follows the playhead (v2.74).** While the progression loops, the bar the keyboard answers for **walks with the playhead**, so a hand already on the keys can answer each bar as it sounds instead of tapping its square first. Armed at the start of every question and **disarmed by the first manual pick** (a square tap, its `▶`, or a web arrow key) — choosing a bar by hand means "I want to work this one", and having the target walk away mid-answer on the next bar boundary is exactly the frustration this must not cause. The **next question re-arms it**, so a disarm never outlives the progression that caused it. Toggle: `Keys follow playhead` in the `Challenge · More tools` dropdown, next to `♪ chords / ♪ notes`.
...
- **Tonic banner.** Shown directly under the bar squares, and **only when the progression does not start at home** (v2.78.2 — `progressionHomeIsObvious`). Three cases, in order:
  - **Its own `I`, or the relative tonic in bar 1** → **nothing at all**: no card, no marker. `vi–V–IV–V` is `i – bVII – bVI – bVII`, a minor vamp that *opens* on its `i`; the ear has an anchor from the first chord, so there is nothing to say. (Flagging it `◆ NO TONIC ◆` was the bug; keying the card on whether it *ended* on the tonic was the half-fix.)
  - **Relative tonic present but not first** (`IV–V–iii–vi` reaches `vi` in bar 4) → **neutral** `◆ READS IN THE RELATIVE MINOR ◆` card (`… MAJOR` for a minor progression carrying `bIII`), whose body says the one useful thing: *"the tonic is not the first chord — it is bar 4, the relative minor's i"*, then the relative reading `bVI – bVII – v – i`. Library/drill marker `◆ relative minor` / `◆ relative major`.
...
- A **per-chord ▶ row** — one `OutlinedButton` per chord, labelled with **plain numbers** (`▶ 1`, `▶ 2`, … positional) rather than Roman numerals, so auditioning a chord doesn't reveal the key's quality. Once revealed, the reveal card (below) shows the Roman-numeral line.
...
- **Meter row** (under Swing): a **bars stepper** (± the number of bars) plus two dropdowns — **time signature** (2/4, 3/4, 4/4, 6/8, …) and **division** (1/4, 1/8, 1/16, 1/32). The default is **2 bars of 2/4 in sixteenths (16 slots)**. Changing any of these reshapes the step grid, the beat/bar separators, and the caption; playback timing is **division-aware** (the slot duration follows the chosen division). A caption summarizes the current meter and slot count.
- **Translate row**: a loop-shift control that rotates the loop by ±n slots with **wrap-around** — `◀` / `▶` shift by one slot, or the user types an amount in a small field and presses **Go** to shift by that many slots. With a **track selected** (tap its name) it rotates **that row alone** and the label names it — `Shift · Tamborim` — so the scope is visible; with nothing selected it shifts the whole loop.
- **Master fader** (in the transport dock, next to the tempo — drum machine only): `🔊` + a short slider + a `nn %` readout, one output level for the step grid, Blocks **and** the metronome click. It is a **monitoring** level, not part of the beat: it is stored per device beside the per-voice trims (reserved key `~master`), so saved/exported beats and the WAV render are unaffected by it. Every other screen's dock omits the block entirely.
- **Grid**: one row per `PercussionInstrument`, each row a step grid sized to the current meter (default 16 cells = 2 bars of 2/4 in sixteenths), with thicker gaps after each beat and a wider bar gap at each bar boundary — both derived from the selected time signature and division. The grid uses **fixed-height rows** (`ROW_HEIGHT_DP`, not weight) so each track's name, `▾` voice popup, and **M/S** toggles always have room and never get clipped in either orientation.
